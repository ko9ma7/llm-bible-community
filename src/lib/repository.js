...
export async function getBackendDiagnostics() {
  if (!isBackendConfigured()) return { configured: false, authReachable: false, schemaReady: false, detail: 'browser config missing', missingTables: [] };
  const supabase = getSupabase();
  if (!supabase) return { configured: true, authReachable: false, schemaReady: false, detail: 'Supabase SDK unavailable', missingTables: [] };
  try {
    const { error: authError } = await supabase.auth.getSession();
    if (authError) return { configured: true, authReachable: false, schemaReady: false, detail: authError.message, missingTables: [] };
    const checks = [
      ['profiles', 'id,role'],
    ...
    for (const [table, columns] of checks) {
      ...
      if (!error) continue;
      if (isMissingSchemaError(error) || /column .* does not exist/i.test(String(error.message || ''))) {
        missingTables.push(table);
        continue;
      }
      // RLS may hide rows, but a permission error still proves the table exists.
      if (String(error.code || '') === '42501') continue;
      return { configured: true, authReachable: true, schemaReady: false, detail: error.message, missingTables };
    }
    if (missingTables.length) return { configured: true, authReachable: true, schemaReady: false, detail: `LLM Bible schema missing: ${missingTables.join(', ')}`, missingTables };
    return { configured: true, authReachable: true, schemaReady: true, detail: 'ready', missingTables: [] };
  } catch (error) {
    return { configured: true, authReachable: false, schemaReady: false, detail: error.message, missingTables: [] };
  }
}
...
function authRedirectUrl() {
  const configured = String(APP_CONFIG.siteUrl || '').replace(/\/$/, '');
  const currentBase = `${window.location.origin}${window.location.pathname.replace(/index\.html$/, '').replace(/\/$/, '')}`;
  const base = configured && !/localhost|127\.0\.0\.1/i.test(configured) ? configured : currentBase;
  return `${base}/#/account`;
}
...
export async function resendSignupConfirmation(email) {
  ...
  if (!address) throw new Error('이메일 주소를 입력해 주세요.');
...
export async function listMySubmissions(userId) {
  ...
  if (error) {
    if (isMissingSchemaError(error)) throw new Error('커뮤니티 DB가 아직 설치되지 않았습니다. setup-permissions.cmd를 실행해 supabase/schema.sql을 적용해 주세요.');
    throw error;
  }
  return data || [];
}
