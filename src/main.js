...
function renderMap() {
  const areas = TAXONOMY.map((area) => {
    const groups = area.groups.map(([name, items]) => `<div class="map-group"><b>${escapeHtml(name)}</b><div>${items.map((item)=>`<span>${escapeHtml(item)}</span>`).join('')}</div></div>`).join('');
    const progress = area.current ? `<span class="framework-progress">현재 ${Number(area.current).toLocaleString()} · 목표 ${escapeHtml(area.target || '확장')}</span>` : `<span class="framework-progress muted">분류 틀 준비</span>`;
...
function renderTools() {
  ...
  return appShell(`${collectionHero('SKILLS · MCP · CONNECTORS','프롬프트 다음 도구를 쉽게 구분합니다.','Skill은 반복 가능한 지식 모듈, Connector/Plugin은 외부 서비스 연결, MCP는 표준화된 도구·리소스 연결, Agent는 여러 단계를 실행하는 런타임으로 나눠 봅니다.')}<section class="section shell tool-choice"><div class="choice-grid"><a href="#/tools?view=skill"><b>Skill</b><span>반복 업무 지침 + 파일 + 도구 묶음</span></a><a href="#/tools?view=connector"><b>Plugin / Connector</b><span>메일·캘린더·GitHub·Figma 등 앱 연결</span></a><a href="#/tools?view=mcp"><b>MCP</b><span>Tool / Resource / Prompt를 표준 방식으로 연결</span></a><a href="#/recipes?cat=agent-architecture"><b>Agent</b><span>계획·도구·검증을 여러 단계로 실행</span></a></div><details class="comparison-details"><summary>기술적 차이 표 보기</summary><div class="table-wrap"><table class="data-table"><thead><tr><th>타입</th><th>목적</th><th>실행 위치</th><th>권한 모델</th></tr></thead><tbody>${toolRows}</tbody></table></div></details></section><section class="section shell explorer-shell"><div class="explorer-layout">${categoryRail('skills','skill')}<div class="explorer-main"><div class="explorer-top"><div><span class="eyebrow">SKILL / PLUGIN SEED</span><h2>업무 영역을 하나씩 열어 보기</h2><p>현재 120개 seed를 기반으로 Design·DevOps·CRM·개인 생산성 등 분류는 틀에 먼저 확보해 두었습니다.</p></div><div class="top-actions"><button id="sourceToggle" class="button ghost small" type="button">원천 소스</button><a class="button ghost small" href="#/map#framework-mcp">MCP 전체 틀</a></div></div><div class="search-panel compact-search"><label class="search-box"><span>⌕</span><input id="skillSearch" type="search" placeholder="PR 리뷰, 논문 비교, GitHub, Jira…"></label><button id="resetSkills" class="text-button" type="button">초기화</button></div><div id="skillLoadNote" class="data-load-note">왼쪽에서 업무 영역을 선택하세요.</div><p id="skillCount" class="result-count"></p><div id="skillGrid" class="research-grid"></div><div id="sourcesPanel" class="sources-panel" hidden></div></div></div></section>`);
}
