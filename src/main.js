...
const rawCurated = window.LLM_BIBLE_DATA || {};
const curated = Object.freeze({
  categories: Array.isArray(rawCurated.categories) ? rawCurated.categories : [],
  roadmap: Array.isArray(rawCurated.roadmap) ? rawCurated.roadmap : [],
  models: Array.isArray(rawCurated.models) ? rawCurated.models : [],
  // v2.0.2 shipped the static corpus under `entries` while the UI expected `knowledge`.
  // Accept both keys so cached/older data files remain compatible after deployment.
  knowledge: Array.isArray(rawCurated.knowledge)
    ? rawCurated.knowledge
    : Array.isArray(rawCurated.entries)
      ? rawCurated.entries
      : []
});
const appState = {
