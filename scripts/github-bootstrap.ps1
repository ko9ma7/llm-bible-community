...
if(-not (Test-Native { gh release view $tag --repo $fullRepo *> $null })) {
  Run "Creating release $tag" { gh release create $tag --repo $fullRepo --title 'LLM Bible Community v2.0.1' --notes 'Supabase Auth, moderated prompt/result submissions, WebP/WebM media pipeline, admin review desk, GitHub Pages deployment.' }
