...
if(-not (Test-Native { gh release view $tag --repo $fullRepo *> $null })) {
  Run "Creating release $tag" { gh release create $tag --repo $fullRepo --title 'LLM Bible Community v2.0.2' --notes 'Fix GitHub Pages native module loading, add boot diagnostics, and harden Supabase placeholder detection.' }
