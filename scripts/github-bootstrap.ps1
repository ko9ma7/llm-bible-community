...
foreach($topic in @('github-pages','llm','prompt-engineering','supabase','ai','rag','mcp','agents','skills')){ & gh repo edit $fullRepo --add-topic $topic *> $null }
...
if(-not (Test-Native { gh release view $tag --repo $fullRepo *> $null })) {
  Run "Creating release $tag" { gh release create $tag --repo $fullRepo --title 'LLM Bible Community v2.1.1' --notes 'Split the 2026-09-21 research corpus into network-friendly category shards, add copy-ready prompt/code/JSON controls, reuse the existing Supabase Auth project, and keep the moderated community workflow.' }
} else { Ok "Release $tag already exists." }

Write-Host "`nDONE" -ForegroundColor Green
Write-Host "Repository: https://github.com/$fullRepo"
Write-Host "Pages:      $deployUrl/"
Write-Host "`nRun backend-bootstrap.cmd once to install the LLM Bible schema in the already-connected Supabase project and promote an administrator account." -ForegroundColor Yellow
