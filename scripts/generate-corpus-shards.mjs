...
const mcpStarters = fs.existsSync(mcpSourcePath) ? JSON.parse(fs.readFileSync(mcpSourcePath, 'utf8')) : [];

const rmTargets = ['prompts','skills','mcp','recipes','failures','index'];
...
// MCP starter shards: lightweight, official-concept-aligned framework records.
const mcpCategoryMeta = {
  core: ['Core concepts', 'Client/Server/Tool/Resource/Prompt와 protocol core'],
  'tool-design': ['Tool design', 'JSON Schema, read/write 분리, retry, idempotency, confirmation'],
  security: ['Auth · Security', 'OAuth scope, least privilege, injection, exfiltration, audit'],
  transport: ['Transport · Operations', 'stdio, Streamable HTTP, stateless scale-out, observability'],
  integrations: ['Integration patterns', 'Filesystem, GitHub, DB, browser, Slack/Jira, Figma, cloud'],
  workflows: ['Workflow patterns', 'Research/Coding agent, router, planner, verifier, HITL']
};
const mcpCategories=[];
for (const category of Object.keys(mcpCategoryMeta)) {
  const [label, description] = mcpCategoryMeta[category];
  const items = mcpStarters.filter(x => x.category === category);
  const file = `data/mcp/${category}.json`;
  writeJson(path.join(root,'public',file), items);
  mcpCategories.push({id:category,label,count:items.length,description,file});
}
const mcpIndex = mcpStarters.map(x => ({
  id:x.id, slug:x.slug, title:x.title, summary:x.summary, category:x.category,
  evidence_level:x.evidence_level, last_verified:x.last_verified, shard:`data/mcp/${x.category}.json`
}));
writeJson(path.join(outRoot,'index','mcp.json'),mcpIndex);
...
const catalog = {
  ...
  collections: {
    ...
    mcp: { total:mcpStarters.length, index:'data/index/mcp.json', categories:mcpCategories },
...
const manifest = {
  ...
  counts: { prompts:(data.prompts||[]).length, skills:(data.skills||[]).length, mcp:mcpStarters.length, recipes:(data.recipes||[]).length, failures:(data.failures||[]).length, sources:(data.sources||[]).length },
...
console.log(`Corpus shards generated: ${manifest.counts.prompts} prompts, ${manifest.counts.skills} skills, ${manifest.counts.mcp} MCP starters, ${manifest.counts.recipes} recipes, ${manifest.counts.failures} failures.`);
