...
const required = [
  ...
  'public/data/catalog.js','public/data/catalog.json','public/data/MANIFEST.json','public/data/index/prompts.json','public/data/index/skills.json','public/data/index/recipes.json','public/data/index/failures.json','public/data/sources.json',
  'public/research/deep-research-report-2026-09-21.md','public/manifest.webmanifest','public/404.html','.github/workflows/deploy.yml','verify-backend.cmd'
];
let failed = false;
...
if (fs.existsSync(catalogPath)) {
  try {
    ...
    for (const name of ['prompts','skills','recipes','failures']) {
...
if (!fs.existsSync(seedManifestPath)) {
  ...
  try {
    ...
    if (Number(seedManifest.total_records) !== 481) { console.error(`[seed-parts] expected 481 records including sources, got ${seedManifest.total_records}`); failed = true; }
