...
> v2.0.1: Windows PowerShell에서 존재하지 않는 GitHub 저장소를 확인할 때 `gh api`의 정상적인 HTTP 404를 치명적 오류로 처리하던 bootstrap 문제를 수정했습니다. 저장소/Pages/Release 등 “존재 여부 확인” 명령은 이제 exit code로 분기하여, 없으면 생성 단계로 계속 진행합니다.
...
## GitHub Pages에서 “LLM Bible을 불러오는 중입니다.”만 보일 때

`v2.0.2`부터 CSS는 `<link rel="stylesheet">`로 로드하고 JavaScript는 native ES module로만 실행합니다. 이전 v2.0.1 배포본은 `main.js`에서 CSS를 직접 import하여 GitHub Pages에서 앱 초기화가 중단될 수 있습니다.

1. v2.0.2 파일을 덮어씁니다.
2. `github-bootstrap.cmd`를 다시 실행합니다.
3. GitHub의 **Actions → Deploy LLM Bible to GitHub Pages**가 초록색 완료가 될 때까지 기다립니다.
4. 브라우저에서 `Ctrl+F5`로 강력 새로고침합니다.

Supabase를 아직 설정하지 않았더라도 홈/지식 라이브러리는 정적 Preview 모드로 정상 표시되어야 합니다.
